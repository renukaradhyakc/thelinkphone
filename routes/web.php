...
Route::get('/call/{userId}', function ($userId) {
    // Validate the user ID format
    if (!preg_match('/^[a-zA-Z0-9]+$/', $userId)) {
        abort(404, 'Invalid CallALink link format');
    }
    ...
    \Log::info("CallALink redirect attempt", ['user_id' => $userId, 'deep_link' => $appDeepLink]);
...
Route::get('/login', function () { return (! Auth::check()) ? \redirect(route('login')) : Redirect::to(getDashboardURL());});

/*
Route::get('/sign-up', function () {
    return redirect()->away('https://app.callalink.com/sign-up');
})->name('sign.up');
...
Route::get('/login', function () {
    return redirect()->away('https://app.callalink.com/login');
})->name('login');
