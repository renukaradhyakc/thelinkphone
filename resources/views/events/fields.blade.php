<div class="row gx-10 mb-5">
        ...
        <div class="input-group mb-5">
            <select name="event_location" class="form-select form-select-solid event-location"
                ...
                @foreach($locationArr as $key => $value)
                    <option value="{{ $key }}" {{($key == old('event_location')) ? 'selected' : ''}} class="update-location">{{ $value }}</option>
                @endforeach
            </select>
    ...
    <div class="col-sm-8">
        ...
        <div class="input-group mb-5 eventLandingLink">
            ...
            {{ Form::text('event_link', null, ['class' => 'form-control eventlink', 'id' => 'eventLinkId', 'placeholder' => __('messages.event.event_landing_page'),'required', 'style' => 'width: 100%;']) }}
        </div>
    </div>
    <div class="col-sm-4">
        <div class="mb-5">
