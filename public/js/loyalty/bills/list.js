...
window.LOY.renderList = function(bills, meta) {
    ...
    body.innerHTML = bills.map(function(b){
        ...
        var av   = window.LOY.avC(b.vendor);
                ...
                '<a href="'+view+'" style="display:flex;align-items:center;gap:10px;text-decoration:none;color:inherit;">' +
                    ...
                    '<div>' +
                        ...
                        '<div class="loy-vmeta">' +
                            ...
                            (b.confidence!=null?'<span>'+window.LOY.confBar(b.confidence)+'</span>':'') +
                        '</div>' +
                    '</div>' +
                '</a>' +

                '<a href="'+view+'" class="col-date text-muted" style="font-size:13px;text-decoration:none;color:inherit;">'+window.LOY.fmtDate(b.bill_date)+'</a>' +

                '<a href="'+view+'" class="col-amount" style="font-weight:700;text-decoration:none;color:inherit;">'+window.LOY.fmt(b.amount)+'</a>' +

                '<a href="'+view+'" class="col-pts loy-pts-pill" style="text-decoration:none;">⭐ '+(b.points||0)+'</a>' +

                '<a href="'+view+'" class="col-status" style="text-decoration:none;">'+window.LOY.badge(b.status)+'</a>' +
...
window.LOY.renderPag = function(meta) {
    ...
    var h = '<button class="loy-pag-btn" onclick="window.LOY.loyBillsLoad('+(cur-1)+')" '+(cur<=1?'disabled':'')+'>‹</button>';

    for (var p=1;p<=last;p++){
        if (p===1 || p===last || (p>=cur-2 && p<=cur+2)) {
            h += '<button class="loy-pag-btn'+(p===cur?' active':'')+'" onclick="window.LOY.loyBillsLoad('+p+')">'+p+'</button>';
        } else if (p===cur-3 || p===cur+3) {
            h += '<span style="padding:0 4px;color:#bbb">…</span>';
        }
    }

    h += '<button class="loy-pag-btn" onclick="window.LOY.loyBillsLoad('+(cur+1)+')" '+(cur>=last?'disabled':'')+'>›</button>';

    pag.innerHTML = h;
};

window.LOY.loyBillsLoad = function(page) {

    window._lp_page = page || 1;

    window.LOY.loyBillsFetch(
        window._lp_page,
        window._lp_status,
        window._lp_sort
    ).then(function(res){
        window.LOY.renderList(res.bills, res.meta);
        window.LOY.renderPag(res.meta);
    });
};