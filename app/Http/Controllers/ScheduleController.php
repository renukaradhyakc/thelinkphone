...
class ScheduleController extends AppBaseController
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (!empty($input['form2']) && count($scheduleSlotTime) > 0) {
        ...
        else if(array_key_exists('form2',$input)){
    ...
    public function addScheduleTimeSlot(Request $request): JsonResponse
    {
        ...
        $checkedDays = $request->input('checked_week_days', []);
        $fromTimes = $request->input('from_time', []);
        $toTimes = $request->input('to_time', []);

        $slots = [];

        // Loop only through checked days
        foreach ($checkedDays as $day) {
            if (!isset($fromTimes[$day]) || !isset($toTimes[$day])) {
                continue; // skip if times missing
            }

            foreach ($fromTimes[$day] as $index => $fromTime) {
                $slots[] = [
                    'day_of_week' => $day,
                    'from_time' => $fromTime,
                    'to_time' => $toTimes[$day][$index] ?? null,
                ];
            }
        }

        // Delete existing slots for this schedule so we don't duplicate old slots
        UserSchedule::where('schedule_id', $request->schedule_id)->delete();
